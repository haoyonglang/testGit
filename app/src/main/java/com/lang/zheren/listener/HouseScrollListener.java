package com.lang.zheren.listener;
...
public class HouseScrollListener implements AbsListView.OnScrollListener {
    //ListView总共显示多少条
    ...
    public HouseScrollListener(View footer, List<HouseInfo> data) {
        this.footer = footer;
        this.data = data;
    }
     ...
     *
     * @param view
     * @param scrollState 1 SCROLL_STATE_TOUCH_SCROLL是拖动  2 SCROLL_STATE_FLING是惯性滑动 0SCROLL_STATE_IDLE是停止 ,
     *                    只有当在不同状态间切换的时候才会执行
     */
    @Override
    ...
    /**
     * 当加载数据完成后，设置加载标志为false表示没有加载数据了   * 并且设置底部加载更多为隐藏
     */
     ...
     * 监听可见界面的情况
     *
     * @param view             ListView   * @param firstVisibleItem 第一个可见的 item 的索引
    ...
    public interface OnloadDataListener {
        void onLoadData(List<HouseInfo> data);
    }
}